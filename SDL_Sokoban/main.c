...
int startJoin(SDL_Window* window, SDL_Surface* wdSurface);
int startCustom(SDL_Window* window, SDL_Surface* wdSurface);
int startHelp(SDL_Window* window, SDL_Surface* wdSurface);
...
int main(int argc, const char* argv[]) {
  ...
  SDL_Window* window = SDL_CreateWindow(
      "DUTY OF SOKOBAN", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
      WIDTH, LENGTH, SDL_WINDOW_ALLOW_HIGHDPI);
  if (window == NULL) {
      ...
      case SDL_KEYDOWN:
        switch (event.key.keysym.sym) {
          ...
          case SDLK_1:
            ...
            SDL_Delay(150);
            continuer = startJoin(window, wdSurface);
            break;
          case SDLK_2:
            // menu seclect 2
            printMenu(window, wdSurface, 2);
            SDL_Delay(150);
            continuer = startCustom(window, wdSurface);
            break;
          case SDLK_3:
            printMenu(window, wdSurface, 3);
            SDL_Delay(150);
            continuer = startHelp(window, wdSurface);
            break;
          default:
            break;
        }
      ...
      case SDL_MOUSEBUTTONDOWN:
        for (int i = 0; i < 3; i++) {
          ...
          if (220 < event.button.x && event.button.x < 380 &&
              260 + i * 105 < event.button.y &&
              event.button.y < 335 + i * 105) {
...
void printMenu(SDL_Window* window, SDL_Surface* wdSurface, int type) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (i == 0)
      SDL_BlitSurface(join[UP], NULL, wdSurface, &pos);
    if (i == 1)
      SDL_BlitSurface(edit[UP], NULL, wdSurface, &pos);
    if (i == 2)
      SDL_BlitSurface(help[UP], NULL, wdSurface, &pos);
  }
  if (type == 1) {
    for (int i = 0; i < 3; i++) {
      pos.x = WIDTH / 2 - 200 / 2;
      pos.y = LENGTH / 2 - 200 / 2 + (i)*105;
      if (i == 0)
        SDL_BlitSurface(join[DOWN], NULL, wdSurface, &pos);
      if (i == 1)
        SDL_BlitSurface(edit[UP], NULL, wdSurface, &pos);
      if (i == 2)
        SDL_BlitSurface(help[UP], NULL, wdSurface, &pos);
  ...
  if (type == 2) {
    for (int i = 0; i < 3; i++) {
      ...
      if (i == 0)
        SDL_BlitSurface(join[UP], NULL, wdSurface, &pos);
      if (i == 1)
        SDL_BlitSurface(edit[DOWN], NULL, wdSurface, &pos);
      if (i == 2)
        SDL_BlitSurface(help[UP], NULL, wdSurface, &pos);
    }
  }
  if (type == 3) {
    for (int i = 0; i < 3; i++) {
      ...
      if (i == 1)
        SDL_BlitSurface(edit[UP], NULL, wdSurface, &pos);
      if (i == 2)
        SDL_BlitSurface(help[DOWN], NULL, wdSurface, &pos);
    }
  }
  SDL_UpdateWindowSurface(window);
}
